...
class BO_algo():
    def __init__(self):
        ...
        self.X = np.random.rand(1,1)*domain[:, 1] #np.array([[1],[3]])
        self.y =  None  #np.asarray([[f(x)] for x in self.X]) # np.random.normal(0,self.sigma_f_noise) 
        self.v =  None #np.asarray([[v(x)] for x in self.X]) # np.random.normal(0,self.sigma_v_noise)
    ...
    def acquisition_function(self, x):
        ...
        af_value = (y[0] + 0.00008*std) * p #0.0002
        #print(af_value,speed)
        """"
    ...
    def get_solution(self):
        ...
        for idx,item in enumerate (self.X):
            #print(item, self.y[idx],self.v[idx])
            if (self.y[idx][0] > y_max) and (self.v[idx][0] >= 1.2): # Hier muss die bound von 1.2 zwingend halten 
                solution = item
                y_max = self.y[idx][0]
                v = self.v[idx]
                y = self.y[idx]

        #print("result: ", solution,y,v)
        return solution 
        #raise NotImplementedError
