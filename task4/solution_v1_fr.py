...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            v = self.v.forward(state).item()
            pi, _ = self.pi.forward(state)
            act = pi.sample().item()
            logp_a = self.pi._log_prob_from_distribution(pi, torch.Tensor([act]))
            return act, v, logp_a
...
class Agent:
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            ret = data['ret']
            tdres = data['tdres']
            logp = data['logp']  # die sind irgendwie NAN

            # Maximimise log likelihood of given action
            loss = -(self.ac.pi.forward(obs, act)[1] * tdres).mean()  # here we have to use the TD
            loss.backward()
            pi_optimizer.step()
            ...
            for _ in range(100):
                ...
                # compute a loss for the value function, call loss.backward() and then v_optimizer.step()
                v_loss = ((self.ac.v.forward(obs) - ret) ** 2).mean()
                v_loss.backward()
                v_optimizer.step()
    ...
    def get_action(self, obs):
        ...
        a, _, _ = self.ac.step(torch.as_tensor(obs, dtype=torch.float32))
